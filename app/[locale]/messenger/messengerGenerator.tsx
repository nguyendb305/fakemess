...
// Helper function to check if a message is the last user message
const isLastUserMessage = (messages: Array<{ role: string; text: string }>, index: number) => {
  if (!messages[index].text || messages[index].role !== "user") return false;
  
  // Check if there are any user messages after this one
  for (let i = index + 1; i < messages.length; i++) {
    if (messages[i].role === "user" && messages[i].text) return false;
  }
  return true;
};
...
const MessengerGenerator = () => {
  ...
  const [activeStatus, setActiveStatus] = useState(t("table1.activeStatusPlaceholder"));
  const [seenText, setSeenText] = useState(t("table1.seenStatusPlaceholder"));
    ...
    >
      ...
      <div className="flex-1 p-1.5 bg-white overflow-y-auto h-[323px] relative">
        {messages.map((message, index) => {
          ...
          const isLastUserMsg = isLastUserMessage(messages, index);
        ...
        <div className="container p-4">
          ...
          <div className="md:flex md:space-x-4 md:gap-4">
            ...
            <div className="md:w-1/2 mb-4 bg-gray-50 p-4 border border-gray-200 shadow-xs rounded-md">
              ...
              <div className="my-4">
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium">{t("table1.seenStatus")}</label>
                  <Switch
                    checked={showSeen}
                ...
                {showSeen && (
                  <Input
                    ...
                    placeholder={t("table1.seenStatusPlaceholder")}
                    className="bg-white"
                  />
                )}
              </div>
